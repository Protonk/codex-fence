...
| `dynamic_probe_contract_accepts_fixture` | Runs `tools/validate_contract_gate.sh --probe …` (dynamic gate) against the fixture probe to keep the stub parser aligned with emit-record flags. | Keep in sync with contract gate behavior and jq-free emit patterns. |
