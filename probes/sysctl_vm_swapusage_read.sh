...
# why: vm.swapusage reveals system resource details that may be gated alongside other sensitive sysctl keys
repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." >/dev/null 2>&1 && pwd)
emit_record_bin="${repo_root}/bin/emit-record"

run_mode="${FENCE_RUN_MODE:-baseline}"
probe_name="sysctl_vm_swapusage_read"
probe_version="1"
...
target_key="${FENCE_SYSCTL_SWAP_KEY:-vm.swapusage}"
...
if [[ ! -x "${sysctl_bin}" ]]; then
  status="error"
  errno_value="ENOENT"
  message="sysctl binary missing"
  raw_exit_code="1"
  stdout_text=""
  stderr_text="sysctl bin ${sysctl_bin} not executable"
else
  set +e
...
"${emit_record_bin}" \
  ...
  --payload-raw-field-json "$(json_key "exit_code")" "${raw_exit_code:-0}" \
