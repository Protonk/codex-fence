...
- Use the supported workflows: `tools/contract_gate/static_gate.sh --probe <id>` (or `make probe PROBE=<id>`) for tight probe loops, `codex-fence --test` to sweep every probe contract, `cargo test --test second_tier` for guard rails, and `make matrix` when you need to exercise probes across modes.
